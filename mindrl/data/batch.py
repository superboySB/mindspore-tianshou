...
def _to_array_with_correct_type(obj: Any) -> np.ndarray:
    ...
    if obj_array.dtype == object:
        ...
        elif any(isinstance(arr, ms.Tensor) for arr in obj_array.reshape(-1)):
...
def _create_value(
        ...
        size: int,
        stack: bool = True,
) -> Union["Batch", np.ndarray, ms.Tensor]:
    ...
    :param bool stack: whether to stack or to concatenate. E.g. if inst has shape of
        ...
        otherwise (10, 5)
    """
    has_shape = isinstance(inst, (np.ndarray, ms.Tensor))
    ...
    elif isinstance(inst, ms.Tensor):
        return ms.numpy.full(shape, fill_value=0, dtype=inst.dtype)
...
def _parse_value(obj: Any) -> Optional[Union["Batch", np.ndarray, ms.Tensor]]:
    if isinstance(obj, Batch):  # most often case
        return obj
    elif (isinstance(obj, np.ndarray) and
          issubclass(obj.dtype.type, (np.bool_, np.number))) or \
            isinstance(obj, ms.Tensor) or obj is None:  # third often case
        return obj
        ...
        if not isinstance(obj, np.ndarray) and \
                ...
                all(isinstance(element, ms.Tensor) for element in obj):
            try:
...
class Batch:
    ...
    def __setitem__(self, index: Union[str, IndexType], value: Any) -> None:
        ...
        for key, val in self.items():
            try:
                ...
                elif isinstance(val, ms.Tensor) or \
        ...
        for batch_key, obj in self.items():
            if isinstance(obj, ms.Tensor):
                if dtype is not None and obj.dtype != dtype:
                    obj = ms.Tensor(obj, dtype=dtype)
                    self.__dict__[batch_key] = obj
            elif isinstance(obj, Batch):
                obj.to_mindspore(dtype, device)
            else:
                # ndarray or scalar
                if not isinstance(obj, np.ndarray):
                    obj = np.asanyarray(obj)
                obj = ms.Tensor.from_numpy(obj)
    ...
    def __cat(self, batches: Sequence[Union[dict, "Batch"]], lens: List[int]) -> None:
        ...
        for key, shared_value in zip(keys_shared, values_shared):
            ...
            elif all(isinstance(element, ms.Tensor) for element in shared_value):
                self.__dict__[key] = ops.Concat()(shared_value)
            else:
    ...
    def stack_(self, batches: Sequence[Union[dict, "Batch"]], axis: int = 0) -> None:
        ...
        for shared_key, value in zip(keys_shared, values_shared):
            # second often
            if all(isinstance(element, ms.Tensor) for element in value):
                self.__dict__[shared_key] = ops.Stack(axis)(value)
            # third often
    ...
    def empty_(self, index: Optional[Union[slice, IndexType]] = None) -> "Batch":
        ...
        """
        for batch_key, obj in self.items():
            if isinstance(obj, ms.Tensor):  # most often case
