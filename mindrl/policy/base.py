...
class BasePolicy(ABC, nn.Cell):
    ...
    :class:`~mindrl.policy.BasePolicy`.

    A policy class typically has the following parts:

    * :meth:`~mindrl.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~mindrl.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~mindrl.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~mindrl.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~mindrl.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~mindrl.policy.BasePolicy.update`: the main interface for training, \
    ...
    Since :class:`~mindrl.policy.BasePolicy` inherits ``torch.nn.Module``, you can
    use :class:`~mindrl.policy.BasePolicy` almost the same as ``torch.nn.Module``,
    ...
    def exploration_noise(self, act: Union[np.ndarray, Batch],
        ...
        """Modify the action from policy.construct with exploration noise.

        :param act: a data batch or numpy.ndarray which is the action taken by
            policy.forward.
        :param batch: the input batch for policy.construct, kept for advanced usage.
    ...
    def soft_update(self, tgt: nn.Cell, src: nn.Cell, tau: float) -> None:
    ...
    def construct(
        ...
        **kwargs: Any,
        ...
        :return: A :class:`~mindrl.data.Batch` which MUST have the following keys:
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        This function is called in :meth:`~mindrl.data.Collector.collect` and only
    ...
    def map_action_inverse(
        ...
        This function is called in :meth:`~mindrl.data.Collector.collect` for
    ...
    def compute_nstep_return(
        ...
        terminal = indices[-1]
        target_q_mindspore = ops.stop_gradient(target_q_fn(buffer, terminal))  # (bsz, ?)
        target_q = to_numpy(target_q_mindspore.reshape(bsz, -1))
        target_q = target_q * BasePolicy.value_mask(buffer, terminal).reshape(-1, 1)
        end_flag = buffer.done.copy()
        end_flag[buffer.unfinished_index()] = True
        target_q = _nstep_return(rew, end_flag, target_q, indices, gamma, n_step)

        batch.returns = to_mindspore_as(target_q, target_q_mindspore)
        if hasattr(batch, "weight"):  # prio buffer update
            batch.weight = to_mindspore_as(batch.weight, target_q_mindspore)
        return batch
