...
class DiscreteBCQPolicy(DQNPolicy):
    ...
    :param torch.nn.Module model: a model following the rules in
        :class:`~mindrl.policy.BasePolicy`. (s -> q_value)
    :param torch.nn.Module imitator: a model following the rules in
        :class:`~mindrl.policy.BasePolicy`. (s -> imitation_logits)
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        act = to_mindspore(batch.act, dtype=torch.long, device=target_q.device)
