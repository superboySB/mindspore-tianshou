...
from mindrl.data import Batch, to_mindspore, to_mindspore_as
...
class DiscreteCRRPolicy(PGPolicy):
    ...
    .. seealso::
        Please refer to :class:`~mindrl.policy.PGPolicy` for more detailed
        explanation.
    """
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:  # type: ignore
        ...
        act = to_mindspore(batch.act, dtype=torch.long, device=q_t.device)
        ...
        with torch.no_grad():
            ...
            rew = to_mindspore_as(batch.rew, q_t_target)
