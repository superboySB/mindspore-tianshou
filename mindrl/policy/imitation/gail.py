...
class GAILPolicy(PPOPolicy):
    ...
    .. seealso::
        ...
        Please refer to :class:`~mindrl.policy.PPOPolicy` for more detailed
        explanation.
    """
    ...
    def disc(self, batch: Batch) -> torch.Tensor:
        obs = to_mindspore(batch.obs, device=self.disc_net.device)
        act = to_mindspore(batch.act, device=self.disc_net.device)
