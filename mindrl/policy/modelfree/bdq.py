...
from mindrl.data import Batch, ReplayBuffer, to_numpy, to_mindspore, to_mindspore_as
from mindrl.policy import DQNPolicy
from mindrl.utils.net.common import BranchingNet
...
class BranchingDQNPolicy(DQNPolicy):
    ...
    def _target_q(self, buffer: ReplayBuffer, indices: np.ndarray) -> torch.Tensor:
        ...
        if self._is_double:
            ...
            act = to_mindspore(act, dtype=torch.long, device=target_q.device)
        else:
    ...
    def _compute_return(
        ...
        batch.returns = to_mindspore_as(target_q, target_q_torch)
        if hasattr(batch, "weight"):  # prio buffer update
            batch.weight = to_mindspore_as(batch.weight, target_q_torch)
        return batch
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        act = to_mindspore(batch.act, dtype=torch.long, device=batch.returns.device)
        q = self(batch).logits
