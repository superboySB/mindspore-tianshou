...
def miniblock(
    ...
    activation: Optional[CellType] = None,
...
class Net(nn.Cell):
    ...
    :param bool dueling_param: whether to use dueling network to calculate Q
        ...
        class:`~mindrl.utils.net.common.MLP`. Default to None.
    :param linear_layer: use this module as linear layer. Default to nn.Linear.

    .. seealso::

        Please refer to :class:`~mindrl.utils.net.common.MLP` for more
        detailed explanation on the usage of activation, norm_layer, etc.

        You can also refer to :class:`~mindrl.utils.net.continuous.Actor`,
        :class:`~mindrl.utils.net.continuous.Critic`, etc, to see how it's
        suggested be used.
    """
    ...
    def construct(
        ...
        if self.use_dueling:  # Dueling DQN
            ...
            logits = q - q.mean(axis=1, keep_dims=True) + v
...
class Recurrent(nn.Cell):
    ...
    def __init__(
        ...
        self.fc1 = nn.Dense(int(np.prod(state_shape)), hidden_layer_size)
        self.fc2 = nn.Dense(hidden_layer_size, int(np.prod(action_shape)))
    ...
    def forward(
        ...
        return obs, {
            "hidden": ops.stop_gradient(hidden.transpose(1, 0,2)),
            "cell": ops.stop_gradient(cell.transpose(1, 0,2))
        }
...
class ActorCritic(nn.Cell):
    ...
    def __init__(self, actor: nn.Cell, critic: nn.Cell) -> None:
        super().__init__()
        self.actor = actor
        self.critic = critic
...
class EnsembleLinear(nn.Cell):
    ...
    def __init__(
        ...
        weight_data = ops.UniformReal()((ensemble_size, in_feature, out_feature))* 2 * k - k
        self.weight = ms.Parameter(weight_data, requires_grad=True)

        self.bias: Union[ms.Parameter, None]
        if bias:
            bias_data = ops.UniformReal()((ensemble_size, 1, out_feature)) * 2 * k - k
            self.bias = ms.Parameter(bias_data, requires_grad=True)
        else:
            self.bias = None
...
class BranchingNet(nn.Cell):
    ...
    def construct(
        ...
        value_out = ops.expand_dims(value_out, 1)
        action_out = []
        for b in self.branches:
            action_out.append(b(common_out))
        action_scores = ops.stack(action_out, 1)
        action_scores = action_scores - ops.ReduceMean(keep_dims=True)(action_scores, axis=2)
        ...
        class new_net_class(net_class):
            ...
            def forward(
                ...
                **kwargs,
                ...
                return super().construct(preprocess_obs(obs), *args, **kwargs)
