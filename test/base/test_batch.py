...
from mindrl.data import Batch, to_numpy, to_mindspore
...
def test_batch_over_batch_to_torch():
    ...
    batch.to_mindspore(dtype=torch.float32)
...
def test_utils_to_torch_numpy():
    ...
    a_torch_float = to_mindspore(batch.a, dtype=torch.float32)
    assert a_torch_float.dtype == torch.float32
    a_torch_double = to_mindspore(batch.a, dtype=torch.float64)
    assert a_torch_double.dtype == torch.float64
    batch_torch_float = to_mindspore(batch, dtype=torch.float32)
    ...
    assert np.allclose(to_numpy(to_mindspore(data_array)), data_array)
    ...
    assert isinstance(to_mindspore(1), torch.Tensor)
    if sys.platform in ["win32", "cygwin"]:  # windows
        assert to_mindspore(1).dtype == torch.int32
    else:
        assert to_mindspore(1).dtype == torch.int64
    assert to_mindspore(1.).dtype == torch.float64
    assert isinstance(to_mindspore({'a': [1]})['a'], torch.Tensor)
    with pytest.raises(TypeError):
        to_mindspore(None)
    with pytest.raises(TypeError):
        to_mindspore(np.array([{}, '2']))
